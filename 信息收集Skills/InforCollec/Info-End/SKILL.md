...
10. 当用户要求审查或修复 Skills 包时，调用 `08-skill-adversarial-audit-regression`，并运行 `python3 -m pytest -q tests` 与 `bash scripts/run-package-selftest.sh . <outdir>`。
...
本入口已把隐藏信息收集、API 规格提取、覆盖面自审和反幻觉发布自测作为增强能力接入，但这些增强能力仍然只产生本机授权范围内的候选和证据摘要。所有新增能力必须经过 `rules/`、`scripts/`、`templates/`、`tests/` 或 `manifests/` 绑定，不能只停留在文档承诺。
