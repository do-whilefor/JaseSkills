...
def run_hidden(tmp_path):
    ...
    subprocess.run(['python3', str(ROOT/'scripts/hidden-info-miner.py'), str(ROOT/'tests/fixtures/hidden_info_app'), '-o', str(out)], check=True)
...
def test_hidden_info_miner_dry_run(tmp_path):
    proc=subprocess.run(['python3', str(ROOT/'scripts/hidden-info-miner.py'), str(ROOT/'tests/fixtures/hidden_info_app'), '--dry-run'], text=True, capture_output=True, check=True)
