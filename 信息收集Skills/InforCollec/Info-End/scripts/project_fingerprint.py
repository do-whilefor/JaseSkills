...
def main():
    ...
    items=[]; counts={}; frameworks=set(); managers=set(); builds=set(); dirs=set()
    ...
    return output_report(args,'project_fingerprint',items,{'summary':{'languages':counts,'frameworks':sorted(frameworks),'package_managers':sorted(managers),'build_tools':sorted(builds)}})
