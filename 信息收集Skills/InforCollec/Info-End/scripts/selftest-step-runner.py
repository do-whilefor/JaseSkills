...
def run_step(name: str, cmd: list[str], root: Path, outdir: Path, timeout: int) -> dict:
    ...
    import os
    env=os.environ.copy(); env.setdefault('PYTEST_DISABLE_PLUGIN_AUTOLOAD','1')
...
def main() -> int:
    ...
    steps = [
        ...
        ('top-tier-dry-run', ['python3', p('scripts/scope_guard.py'), '--input', p('tests/fixtures/top_tier_info_app'), '--scope', p('tests/fixtures/top_tier_info_app'), '--output', o('top-tier-scope.json'), '--format', 'json', '--dry-run']),
    ]
    results=[]
