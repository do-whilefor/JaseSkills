...
| 能力 | 文件 | 当前实现等级 | 限制 |
|---|---|---|---|
| 隐藏信息候选挖掘 | `scripts/hidden-info-miner.py`, `tests/test_hidden_info_miner.py`, `rules/hidden-info-collection-rules.yaml`, `templates/hidden-info-evidence.md` | 本机只读候选级实现 | 不联网、不验证漏洞、不输出完整 secret；需要动态验证与 QG |
| API 规格深度盘点 | `scripts/api-spec-inventory.py`, `tests/test_api_spec_inventory.py`, `templates/api-spec-inventory.md` | OpenAPI/Swagger/Postman/GraphQL/proto 静态候选 | YAML 为 best-effort；不做字段爆破或 introspection |
| 40 项覆盖面自审 | `scripts/surface-coverage-audit.py`, `tests/test_surface_coverage_audit.py`, `templates/info-collection-audit-report.md` | 文件/脚本/测试/JSONL 信号绑定审计 | 自审只证明包内能力绑定，不证明目标项目已被完整覆盖 |

新增脚本必须与 `README.md`、`CAPABILITY_INDEX.md`、`manifests/skill_package_manifest.json`、`templates/`、`tests/` 保持一致。若脚本、测试或模板缺失，对应能力必须标记为 `partial`、`missing` 或 `unverifiable`。
