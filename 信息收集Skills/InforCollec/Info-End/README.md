...
python3 scripts/hidden-info-miner.py /path/to/project -o hidden-info-candidates.jsonl

# API 规格：OpenAPI/Swagger JSON/YAML、Postman collection、GraphQL schema/operation、protobuf/gRPC
python3 scripts/api-spec-inventory.py /path/to/project -o api-spec-candidates.jsonl

# 40 项信息收集覆盖面自审；可把多个 JSONL 输出喂入做证据信号检查
python3 scripts/surface-coverage-audit.py --root . \
...
```bash
python3 scripts/info_collect_orchestrator.py \
