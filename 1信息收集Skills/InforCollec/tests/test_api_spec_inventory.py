...
def test_api_spec_inventory_extracts_openapi_postman_graphql_grpc(tmp_path):
    ...
    subprocess.run([sys.executable, str(ROOT/'scripts/api-spec-inventory.py'), str(ROOT/'tests/fixtures/api_spec_app'), '-o', str(out)], check=True)
...
def test_api_spec_inventory_dry_run():
    proc=subprocess.run([sys.executable, str(ROOT/'scripts/api-spec-inventory.py'), str(ROOT/'tests/fixtures/api_spec_app'), '--dry-run'], text=True, capture_output=True, check=True)
