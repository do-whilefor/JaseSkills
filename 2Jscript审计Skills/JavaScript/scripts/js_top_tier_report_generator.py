...
def main():
    ...
    for k,v in sorted(counts.items()):
        md.append(f'- `{k}`: {v}')
    md += ['', '## 4. 动态证据、role/tenant 与隐藏参数','']
    md.append(f"- Runtime evidence: `{runtime.get('status','missing')}`")
    md.append(f"- Role/tenant diff: `{diff.get('status','missing')}`")
    md.append(f"- API parameter model: `{api_model.get('status','missing')}`, APIs: `{api_model.get('api_count',0)}`")
    md.append(f"- Backend param diff: `{param_diff.get('status','missing')}`, backend-only fields: `{len(param_diff.get('backend_only_fields',[]))}`")
    md.append(f"- Browser lazyload replay plan: `{browser_plan.get('status','missing')}`")
    md.append(f"- Severe candidate map: `{severe_map.get('status','missing')}`, candidates: `{len(severe_map.get('candidates',[]))}`")
    md += ['', '## 5. 前 30 条候选','']
    ...
    api_html = html.escape(json.dumps({'api_count': api_model.get('api_count',0), 'diff': param_diff.get('decision','missing'), 'severe_candidates': len(severe_map.get('candidates',[])), 'browser_plan': browser_plan.get('status','missing')}, ensure_ascii=False, indent=2))
    ...
    page += '<h2>API/Hidden Params/Replay</h2><pre>' + api_html + '</pre>'
