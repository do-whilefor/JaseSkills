...
PARAMETER_RE = re.compile(r"""
    (?:req\.(?:query|body|params|headers|cookies)\.[A-Za-z0-9_.$\[\]'\"]+)|
    (?:request\.(?:args|form|json|get_json|headers|cookies|files))|
    (?:@(?:RequestParam|PathVariable|RequestBody|RequestHeader|CookieValue)\b)|
    (?:params\[(?:'|\")[^'\"]+(?:'|\")\])|
    (?:request\.GET|request\.POST|request\.FILES)|
    (?:ctx\.(?:Query|Body|Params|Cookies|GetHeader))
""", re.I | re.X)
SENSITIVE_PARAM_RE = re.compile(r"(tenant|org|workspace|owner|user_id|role|admin|scope|permission|status|state|price|amount|quantity|discount|coupon|redirect|return_to|callback|url|uri|path|file|filename|template|command|query|filter|sort|limit|include|expand)", re.I)
ENTRYPOINT_KIND_RULES = [
    ('http_route', re.compile(r"(@(?:app|router)\.(?:get|post|put|patch|delete)\(['\"]([^'\"]+)|\.(?:get|post|put|patch|delete)\(['\"]([^'\"]+)|Route\(['\"]([^'\"]+)|path\(['\"]([^'\"]+))", re.I)), ('graphql_schema_or_resolver', re.compile(r'(graphql|resolver|schema\.graphql|type Query|type Mutation)', re.I)),
    ('websocket_handler', re.compile(r'(websocket|socket\.io|ws\.on|channels|consumer)', re.I)),
    ('queue_job_worker', re.compile(r'(queue|worker|job|consumer|bullmq|celery|sidekiq|resque|rabbit|kafka|sqs)', re.I)),
    ('cli_command', re.compile(r'(click\.command|argparse|commander|cobra|artisan|rake|thor|console command)', re.I)),
    ('cron_task', re.compile(r'(cron|schedule|crontab|apscheduler|sidekiq-cron)', re.I)),
    ('webhook_callback', re.compile(r'(webhook|callback|redirect_uri|return_to|relay_state)', re.I)),
    ('upload_download_import_export', re.compile(r'(upload|download|import|export|multipart|sendfile|attachment)', re.I)),
    ('admin_internal_debug', re.compile(r'(admin|internal|debug|actuator|metrics|health)', re.I)),
]
...
def extract(root: Path) -> dict[str, Any]:
    ...
    signals: dict[str, list] = {
        ...
        'storybook_mock_e2e_flows': [], 'routes': [], 'dependency_semantics': [], 'config_semantics': [], 'lockfile_version_reachability': [], 'build_script_risks': [], 'parameter_semantics': [], 'sensitive_parameter_candidates': [], 'entrypoint_matrix': [], 'security_context': [], 'relationship_seed_edges': []
    }
    for p in sorted(root.rglob('*')):
        ...
        for m in PARAMETER_RE.finditer(text):
            ...
            kind = 'unknown'
            low = raw.lower()
            if 'query' in low or 'args' in low or 'requestparam' in low: kind = 'query'
            elif 'body' in low or 'json' in low or 'form' in low or 'requestbody' in low: kind = 'body'
            elif 'params' in low or 'pathvariable' in low: kind = 'path'
            elif 'header' in low: kind = 'header'
            elif 'cookie' in low: kind = 'cookie'
            elif 'file' in low or 'multipart' in low: kind = 'multipart'
            param_name = raw.split('.')[-1].strip("[]'\"()")[:80]
            row = {'path': rel, 'kind': kind, 'raw': raw[:160], 'param_name': param_name, 'line': text[:m.start()].count('\n')+1, 'evidence_hash': sha16(raw)}
            signals['parameter_semantics'].append(row)
            if SENSITIVE_PARAM_RE.search(raw): signals['sensitive_parameter_candidates'].append(row)
        combined = rel + '\n' + text[:50000]
        for ep_kind, ep_rx in ENTRYPOINT_KIND_RULES:
            if ep_rx.search(combined):
                signals['entrypoint_matrix'].append({'path': rel, 'entrypoint_kind': ep_kind, 'evidence_hash': sha16(ep_kind + rel)})
        for ctx_name, ctx_rx in [('role', re.compile(r'\b(role|admin|member|owner|viewer)\b', re.I)), ('tenant', TENANT_RE), ('workflow_state', re.compile(r'\b(status|state|approved|pending|draft|published|archived|deleted)\b', re.I)), ('billing_tier', re.compile(r'\b(plan|tier|subscription|paid|trial|quota)\b', re.I)), ('feature_flag', FEATURE_RE)]:
            if ctx_rx.search(combined): signals['security_context'].append({'path': rel, 'context': ctx_name, 'evidence_hash': sha16(ctx_name+rel)})
        # Coarse relationship seeds consumed by graph and detector stages; never proof by itself.
        for r in signals['routes'][-20:]:
            if r.get('path') == rel:
                signals['relationship_seed_edges'].append({'from': 'Route', 'to': 'Handler/Middleware/AuthZ/Parameter/Model/Sink/Response', 'path': rel, 'route': r.get('route'), 'claim_policy': 'candidate_seed_only_requires_code_graph_and_dynamic_evidence'})
    return {
