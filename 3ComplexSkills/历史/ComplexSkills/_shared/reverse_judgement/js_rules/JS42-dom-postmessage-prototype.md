# JS42-dom-postmessage-prototype DOM XSS / postMessage origin / 原型污染

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`DOM sink + origin + object merge scanner`。

输出字段：`client_side_security_risks[]`。

候选漏洞联动：`C22`。

进入 manifest 字段：`manifest.candidates`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-dom-postmessage-prototype.json`。
