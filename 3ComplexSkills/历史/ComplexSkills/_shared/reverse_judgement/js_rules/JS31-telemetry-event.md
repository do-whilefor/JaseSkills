# JS31-telemetry-event 埋点事件识别

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`analytics event parser`。

输出字段：`telemetry_events[]`。

候选漏洞联动：`C03,C22`。

进入 manifest 字段：`js_graph.telemetry_events`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-telemetry.json`。
