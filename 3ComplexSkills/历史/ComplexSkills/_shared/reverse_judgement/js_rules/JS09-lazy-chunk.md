# JS09-lazy-chunk lazy chunk

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`chunk lineage parser`。

输出字段：`chunk_lineage[]`。

候选漏洞联动：`C03,C22`。

进入 manifest 字段：`js_graph.chunk_lineage`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-lazy-chunk.json`。
