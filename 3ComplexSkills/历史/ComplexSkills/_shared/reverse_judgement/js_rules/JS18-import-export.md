# JS18-import-export import/export 图

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`AST symbol graph`。

输出字段：`symbol_graph.import_export_edges[]`。

候选漏洞联动：`C03,C22`。

进入 manifest 字段：`js_graph.symbol_graph`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-import-export.json`。
