...
AST / parser 要求：`external URL classifier, no third-party fetch`。

输出字段：`cdn_references[]`。

候选漏洞联动：`C22`。

进入 manifest 字段：`js_sources.cdn_references`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-cdn-ref.json`。
