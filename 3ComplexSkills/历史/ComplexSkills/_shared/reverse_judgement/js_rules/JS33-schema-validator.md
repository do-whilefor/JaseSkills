# JS33-schema-validator zod/yup/joi/ajv schema 识别

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`validator AST pattern parser`。

输出字段：`client_schema_validators[]`。

候选漏洞联动：`C14,C15,C16`。

进入 manifest 字段：`js_graph.client_schema_validators`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-schema-validator.json`。
