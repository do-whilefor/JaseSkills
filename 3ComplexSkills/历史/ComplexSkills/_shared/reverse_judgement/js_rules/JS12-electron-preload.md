# JS12-electron-preload Electron preload

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`contextBridge/ipcRenderer scanner`。

输出字段：`electron_preload[]`。

候选漏洞联动：`C06,C22`。

进入 manifest 字段：`js_graph.electron_preload`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-electron-preload.json`。
