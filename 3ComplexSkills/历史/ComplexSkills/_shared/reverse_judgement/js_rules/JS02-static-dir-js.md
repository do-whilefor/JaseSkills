# JS02-static-dir-js 静态目录 JS

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`filesystem collector`。

输出字段：`js_files[]`。

候选漏洞联动：`C22,C03`。

进入 manifest 字段：`js_sources.static_files`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-static-dir.json`。
