# JS41-js-ssrf-redirect-cors SSRF redirect callback CORS 风险

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`URL param/callback/origin/CORS client hints`。

输出字段：`url_boundary_risks[]`。

候选漏洞联动：`C13,C20,C21`。

进入 manifest 字段：`manifest.candidates`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-url-boundary.json`。
