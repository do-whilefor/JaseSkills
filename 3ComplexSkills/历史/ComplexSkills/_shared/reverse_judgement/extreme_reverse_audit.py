...
def exists(rel):
    ...
    if p.exists():
        return True
    # historical selftest result names are stored under _shared/runs in the packaged Skill
    if rel in {'SELFTEST_E2E_REPLAY_RESULT.json','SELFTEST_TOOL_HEALTH_RESULT.json'}:
        return (ROOT / '_shared' / 'runs' / rel).exists()
    return False
