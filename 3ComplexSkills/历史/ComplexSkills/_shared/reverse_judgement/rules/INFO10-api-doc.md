# INFO10-api-doc API 文档识别

负责 skill：`skills/02-project-intelligence/SKILL.md`

必须执行规则：`api_doc_path,source_type,declared_route,auth_hint`。

输出字段：`project_inventory.api_docs[]`。

进入 manifest 字段：`project_inventory_v2.api_docs`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/api-docs-openapi.json`。
