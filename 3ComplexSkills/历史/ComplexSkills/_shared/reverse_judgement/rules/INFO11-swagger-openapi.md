# INFO11-swagger-openapi Swagger / OpenAPI 识别

负责 skill：`skills/02-project-intelligence/SKILL.md`

必须执行规则：`openapi_version,paths,security_schemes,servers`。

输出字段：`project_inventory.openapi[]`。

进入 manifest 字段：`project_inventory_v2.api_docs`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/openapi-security-schemes.json`。
