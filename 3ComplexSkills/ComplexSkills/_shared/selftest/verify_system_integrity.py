...
ERRORS: list[str] = []
WARNINGS: list[str] = []
REF_RE = re.compile(r"(?:(?:_shared/|skills/|reverse_audit_reports/)[A-Za-z0-9_./:\-\[\]]+(?:\.md|\.json|\.csv|\.yaml|\.yml|\.py|\.sh|\.ps1|\.ts|\.tsx|\.js)?)")
try:
...
def check_references() -> None:
    for p in sorted(ROOT.rglob("*")):
        ...
        for ref in REF_RE.findall(text):
            ...
            # Ignore regex/glob examples such as _shared/tests/fixtures/C[0-9][0-9]-*.json.
            if '[0-9]' in ref_clean or ref_clean.endswith(('Dxx-', '/C', '/D')):
                continue
...
def main() -> int:
    ...
    for step in STEP_FUNCTION_NAMES:
        ...
        try:
            ...
            ERRORS.append(f'{step} raised {exc.__class__.__name__}: {exc}')
        result = {'step': step, 'passed': not ERRORS, 'errors': list(ERRORS), 'warnings': list(WARNINGS)}
