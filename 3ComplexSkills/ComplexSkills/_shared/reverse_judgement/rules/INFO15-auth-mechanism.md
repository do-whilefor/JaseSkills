# INFO15-auth-mechanism 认证机制识别

负责 skill：`skills/04-attack-surface-mapping/SKILL.md`

必须执行规则：`session,jwt,oauth,saml,api_key,magic_link,sso`。

输出字段：`attack_surface.auth_mechanisms[]`。

进入 manifest 字段：`surface_manifest_v2.auth`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/auth-mechanisms.json`。
