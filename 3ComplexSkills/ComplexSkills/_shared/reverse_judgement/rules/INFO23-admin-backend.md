# INFO23-admin-backend 管理后台识别

负责 skill：`skills/04-attack-surface-mapping/SKILL.md`

必须执行规则：`admin_route,admin_menu_key,guard,role_required`。

输出字段：`attack_surface.admin_surfaces[]`。

进入 manifest 字段：`surface_manifest_v2.admin`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/admin-shadow-route.json`。
