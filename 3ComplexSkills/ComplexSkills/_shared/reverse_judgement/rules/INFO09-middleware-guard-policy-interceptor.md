# INFO09-middleware-guard-policy-interceptor middleware / guard / policy / interceptor 识别

负责 skill：`skills/03-code-knowledge-graph/SKILL.md`

必须执行规则：`authn_guard,authz_guard,policy,interceptor_order,missing_guard_marker`。

输出字段：`security_graph.auth_chain[]`。

进入 manifest 字段：`security_graph_v3.edges`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/middleware-order-bypass.json`。
