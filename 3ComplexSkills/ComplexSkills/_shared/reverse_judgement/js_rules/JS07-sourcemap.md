# JS07-sourcemap sourcemap

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`sourcemap parser + sourcesContent reentry`。

输出字段：`source_map_reentry[]`。

候选漏洞联动：`C03,C22,C20`。

进入 manifest 字段：`js_sources.sourcemaps`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-sourcemap.json`。
