# JS05-manifest-js manifest JS

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`asset manifest parser`。

输出字段：`manifest_entries[]`。

候选漏洞联动：`C03,C22`。

进入 manifest 字段：`js_sources.manifest_entries`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-manifest.json`。
