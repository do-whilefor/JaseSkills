# JS35-secret-leak 敏感信息/API/internal host/token/key/secret/bucket/DSN 泄露

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`secret detector + example token negative control`。

输出字段：`sensitive_strings[]`。

候选漏洞联动：`C22`。

进入 manifest 字段：`js_graph.sensitive_strings`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-secret-leak.json`。
