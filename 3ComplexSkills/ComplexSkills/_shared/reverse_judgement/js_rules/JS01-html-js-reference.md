# JS01-html-js-reference HTML 引用 JS

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`HTML parser + script src extraction`。

输出字段：`html_js_references[]`。

候选漏洞联动：`C22,C03`。

进入 manifest 字段：`js_sources.html_references`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-html-ref.json`。
