# JS34-graphql-fragment-persisted GraphQL fragment/operation/persisted query hash 识别

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`GraphQL document + hash parser`。

输出字段：`graphql_operations[]`。

候选漏洞联动：`C17`。

进入 manifest 字段：`js_graph.graphql_operations`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-graphql-persisted.json`。
