# JS04-built-assets-js 构建产物 JS

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`bundle classifier`。

输出字段：`bundle_assets[]`。

候选漏洞联动：`C03,C22`。

进入 manifest 字段：`js_sources.bundle_assets`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-bundles.json`。
