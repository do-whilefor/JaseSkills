# JS44-graphql-auth-risk GraphQL 权限风险

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`operation + fragment + object ID auth join`。

输出字段：`graphql_access_risks[]`。

候选漏洞联动：`C17`。

进入 manifest 字段：`manifest.candidates`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-graphql-auth-risk.json`。
