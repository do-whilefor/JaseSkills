...
def cmd_status(cmd: str, args: list[str] | None = None, display: str | None = None) -> dict:
    args = args or ['--version']
    exe = shutil.which(cmd) if cmd != sys.executable else sys.executable
    name = display or cmd
    if not exe:
        return {'name': name, 'status': 'missing', 'path': None, 'detail': f'{cmd} not found'}
    try:
        cp = subprocess.run([exe, *args], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, timeout=5)
        detail = (cp.stdout or cp.stderr).splitlines()[0] if (cp.stdout or cp.stderr) else 'version command returned no output'
        status = 'ready' if cp.returncode == 0 else 'degraded'
    except Exception as exc:
        detail = str(exc); status = 'degraded'
    return {'name': name, 'status': status, 'path': exe, 'detail': detail}


def node_pkg(pkg: str) -> dict:
    node = shutil.which('node')
    if not node:
        return {'name': pkg, 'status': 'missing', 'detail': 'node missing'}
...
def playwright_status() -> dict:
    ...
    try:
        ...
        out['errors'].append(str(exc)[:1000])
    return out


def main() -> int:
    ap = argparse.ArgumentParser(); ap.add_argument('--out'); args = ap.parse_args()
    checks = [cmd_status(sys.executable, display='python_current')]
    if os.name == 'nt':
        checks += [cmd_status('python'), cmd_status('py', ['-3', '--version'], 'py_launcher')]
    else:
        checks += [cmd_status('python3'), cmd_status('python')]
    command_specs = [
        ('node', ['--version'], 'node'),
        ('java', ['-version'], 'java'),
        ('php', ['--version'], 'php'),
    ...
    checks += [cmd_status(cmd, args, display) for cmd, args, display in command_specs]
