...
def main():
    ...
    if runtime_ready(runtime): dynamic=85
    ...
    if backend_acceptance_ready(acceptance): backend_diff=85
    elif param_diff.get('schema_version') == 'js-backend-param-diff/v1':
        backend_diff=25; caps.append('前后端参数差异只有候选，没有后端接受性请求/响应证据')
        blocking.append('backend accepted hidden params not proven')
    else:
        backend_diff=0; caps.append('缺少前后端参数差异模型')
    ...
    if gql_ws_runtime.get('schema_version') != 'js-graphql-ws-runtime-replay/v1':
        caps.append('缺 GraphQL/WebSocket runtime replay 产物，相关漏洞不得 promoted')
    if oss_registry.get('real_oss_replay_count', 0) < 10:
    ...
    print(json.dumps({'ok': True, 'decision': decision, 'overall_score': overall, 'out': str(d/'js_quality_gate.json')}, ensure_ascii=False, indent=2))
