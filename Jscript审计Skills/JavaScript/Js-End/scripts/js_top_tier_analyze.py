...
def run_node_backend(js_path: Path, backend_script: Path, required_module: str) -> dict[str,Any] | None:
    ...
    if proc.returncode != 0:
        return {'backend': backend_script.name, 'status':'error', 'error':proc.stderr[:4000], 'file':str(js_path), 'returncode':proc.returncode}
    try:
        data=json.loads(proc.stdout)
        data['backend_script']=backend_script.name
        if data.get('status') != 'ready':
            data['status'] = 'ready'
        return data
    except Exception as e:
        return {'backend': backend_script.name, 'status':'error', 'error':f'bad backend json: {e}', 'raw':proc.stdout[:1000], 'file':str(js_path)}

def ast_result_is_success(res: dict[str,Any] | None) -> bool:
    if not isinstance(res, dict):
        return False
    if res.get('status') != 'ready' or res.get('error'):
        return False
    return any(res.get(k) for k in ('imports','calls','memberWrites','propertyAccess','strings','endpoints','interfaces','enums','dynamicImports','graphql','websocket'))
...
def build_findings(ledger: dict[str,Any], root: Path, out: Path, backends: list[BackendStatus]) -> dict[str,Any]:
    ...
    ast_backend_available=any(b.name=='babel_parser' and b.status=='ready' for b in backends) or any(b.name=='typescript_compiler_api' and b.status=='ready' for b in backends)
    for asset in ledger.get('assets',[]):
        if asset.get('kind') in {'javascript','service_worker'}:
            ...
            if ast_backend_available:
                ...
                if res:
                    ast_results.append(res); tried.append(res)
                if not ast_result_is_success(res):
                    res2=run_node_backend(root/asset['path'], ts_script, 'typescript')
                    if res2:
                        ast_results.append(res2); tried.append(res2)
    ...
    ast_successes=[r for r in ast_results if ast_result_is_success(r)]
    ast_errors=[r for r in ast_results if isinstance(r,dict) and r.get('error')]
    semantic_status='ready' if ast_successes else 'candidate-only'
    semantic_warning='' if ast_successes else 'No error-free AST backend result; regex findings remain candidate-only and cannot be called semantic audit.'
    analysis={
        'schema_version':'js-top-tier-analysis/v2',
        'backend_status':[asdict(b) for b in backends],
        'semantic_status':semantic_status,
        'semantic_warning': semantic_warning,
        'ast_success_count': len(ast_successes),
