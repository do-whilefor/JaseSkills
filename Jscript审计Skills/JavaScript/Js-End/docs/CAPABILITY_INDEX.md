...
## 2026-06-06 补强：隐藏接口/隐藏参数/浏览器懒加载/前后端参数差异

新增 Skill：

- `16-js-hidden-api-parameter-modeling`：API 与参数逆向建模，覆盖 REST/GraphQL/WebSocket/SSE/gRPC-web/JSON-RPC、hidden params、backend accepted extra params。
- `17-js-browser-lazyload-replay`：真实浏览器交互与懒加载 replay 计划，覆盖点击、滚动、hover、tab、SPA、多角色、多租户、HAR/trace/screenshot。
- `18-js-backend-accepted-param-diff`：前端传参、TS/schema、HAR、后端 DTO/model/controller 接受字段差异。
- `19-js-severe-vulnerability-verification`：把 JS/API 参数候选映射到严重漏洞验证链，所有未验证项保持 candidate-only。
- `20-js-self-audit-verdict`：自我追责矩阵，阻断 doc-only、regex-only、fixture-only 伪 ready。

新增脚本：`js_api_parameter_model.py`、`js_backend_param_diff.py`、`js_browser_lazyload_replay_plan.py`、`js_severe_js_candidate_mapper.py`、`js_self_audit_matrix.py`。
