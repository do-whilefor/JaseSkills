# JS10-dynamic-import dynamic import

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`AST ImportExpression / heuristic fallback`。

输出字段：`dynamic_imports[]`。

候选漏洞联动：`C03,C22`。

进入 manifest 字段：`js_graph.dynamic_imports`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-dynamic-import.json`。
