# JS23-websocket-sse WebSocket / SSE 识别

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`WebSocket/EventSource constructor scanner`。

输出字段：`realtime_channels[]`。

候选漏洞联动：`C02,C22`。

进入 manifest 字段：`js_graph.realtime_channels`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-websocket-sse.json`。
