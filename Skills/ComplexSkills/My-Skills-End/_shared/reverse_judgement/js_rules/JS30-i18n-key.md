# JS30-i18n-key i18n key 识别

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`i18n key/module inference`。

输出字段：`i18n_signals[]`。

候选漏洞联动：`C03,C05`。

进入 manifest 字段：`js_graph.i18n_signals`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-i18n.json`。
