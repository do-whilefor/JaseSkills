# JS24-postmessage postMessage 识别

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`postMessage origin taint checker`。

输出字段：`postmessage_edges[]`。

候选漏洞联动：`C02,C22`。

进入 manifest 字段：`js_graph.postmessage_edges`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-postmessage.json`。
