# JS06-service-worker service worker

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`service worker AST-lite + cache API scanner`。

输出字段：`service_workers[]`。

候选漏洞联动：`C03,C22`。

进入 manifest 字段：`js_sources.service_workers`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-service-worker.json`。
