# JS32-mock-api mock API 识别

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`MSW/mock/storybook/e2e parser`。

输出字段：`mock_api_signals[]`。

候选漏洞联动：`C03,C22`。

进入 manifest 字段：`js_graph.mock_api_signals`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-mock-api.json`。
