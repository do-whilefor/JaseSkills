# JS26-frontend-route 前端路由识别

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`router config parser`。

输出字段：`frontend_routes[]`。

候选漏洞联动：`C03,C02`。

进入 manifest 字段：`js_graph.frontend_routes`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-frontend-route.json`。
