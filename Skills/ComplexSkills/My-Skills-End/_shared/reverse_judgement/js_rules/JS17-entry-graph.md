# JS17-entry-graph 入口关系图

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`entrypoint mapper`。

输出字段：`entrypoints[]`。

候选漏洞联动：`C03,C22`。

进入 manifest 字段：`js_graph.entrypoints`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-entry-graph.json`。
