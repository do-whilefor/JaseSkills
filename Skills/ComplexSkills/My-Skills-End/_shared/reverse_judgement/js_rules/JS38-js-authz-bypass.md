# JS38-js-authz-bypass JS 权限绕过候选点

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`permission key/API endpoint guard mismatch`。

输出字段：`candidate_to_manifest_links[]`。

候选漏洞联动：`C02`。

进入 manifest 字段：`manifest.candidates`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-authz.json`。
