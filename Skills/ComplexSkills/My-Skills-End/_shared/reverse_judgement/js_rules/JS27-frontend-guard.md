# JS27-frontend-guard 前端权限守卫识别

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`router guard + role check parser`。

输出字段：`frontend_permissions[]`。

候选漏洞联动：`C02,C03,C04`。

进入 manifest 字段：`js_graph.frontend_permissions`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-frontend-guard.json`。
