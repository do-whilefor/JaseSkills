# JS08-preload-prefetch preload / prefetch

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`HTML link rel parser`。

输出字段：`preload_prefetch[]`。

候选漏洞联动：`C03,C22`。

进入 manifest 字段：`js_sources.preload_prefetch`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-preload.json`。
