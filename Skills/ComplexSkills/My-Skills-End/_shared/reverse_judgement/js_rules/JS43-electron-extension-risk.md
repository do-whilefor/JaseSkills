# JS43-electron-extension-risk Electron IPC / extension 跨边界风险

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`IPC/content script/native bridge scanner`。

输出字段：`cross_boundary_risks[]`。

候选漏洞联动：`C06,C22`。

进入 manifest 字段：`manifest.candidates`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-electron-extension-risk.json`。
