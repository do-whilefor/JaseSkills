# JS40-js-file-entrypoints 文件上传读取导出入口

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`file API method/body/content-type detector`。

输出字段：`file_entrypoints[]`。

候选漏洞联动：`C10,C11,C12`。

进入 manifest 字段：`manifest.candidates`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-file-entrypoints.json`。
