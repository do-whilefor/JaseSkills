# JS29-feature-flag feature flag 识别

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`flag name and guarded API mapping`。

输出字段：`feature_flags[]`。

候选漏洞联动：`C03,C22`。

进入 manifest 字段：`js_graph.feature_flags`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-feature-flag.json`。
