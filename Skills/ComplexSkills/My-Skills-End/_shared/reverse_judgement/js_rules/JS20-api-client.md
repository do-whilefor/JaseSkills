# JS20-api-client API client 识别

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`fetch/axios/request/GraphQL client detector`。

输出字段：`api_clients[]`。

候选漏洞联动：`C03,C13,C20`。

进入 manifest 字段：`js_graph.api_clients`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-api-client.json`。
