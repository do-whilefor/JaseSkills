# JS25-oauth-callback-page OAuth callback 页面识别

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`callback route + redirect param scanner`。

输出字段：`oauth_callback_pages[]`。

候选漏洞联动：`C20`。

进入 manifest 字段：`js_graph.oauth_callbacks`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-oauth-callback.json`。
