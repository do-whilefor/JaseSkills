# JS13-extension-scripts 浏览器扩展 content/background script

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`manifest v2/v3 + content script parser`。

输出字段：`browser_extension[]`。

候选漏洞联动：`C22,C02`。

进入 manifest 字段：`js_graph.browser_extension`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-extension.json`。
