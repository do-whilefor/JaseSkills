# INFO03-framework-detection 框架识别

负责 skill：`skills/02-project-intelligence/SKILL.md`

必须执行规则：`framework_signals,dependency_manifest,config_signature`。

输出字段：`project_inventory.frameworks[]`。

进入 manifest 字段：`project_inventory_v2.frameworks`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/framework-mixed-express-django.json`。
