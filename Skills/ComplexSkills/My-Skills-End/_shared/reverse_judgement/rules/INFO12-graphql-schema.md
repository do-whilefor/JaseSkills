# INFO12-graphql-schema GraphQL schema 识别

负责 skill：`skills/03-code-knowledge-graph/SKILL.md`

必须执行规则：`schema_path,type,field,resolver,auth_directive`。

输出字段：`security_graph.graphql_schema[]`。

进入 manifest 字段：`security_graph_v3.nodes`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/graphql-schema-resolver-auth.json`。
