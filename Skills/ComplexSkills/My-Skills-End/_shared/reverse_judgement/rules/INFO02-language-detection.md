# INFO02-language-detection 语言识别

负责 skill：`skills/02-project-intelligence/SKILL.md`

必须执行规则：`language_signals,extension_histogram,parser_probe`。

输出字段：`project_inventory.languages[]`。

进入 manifest 字段：`project_inventory_v2.languages`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/language-polyglot.json`。
