# INFO22-test-account 测试账号识别

负责 skill：`skills/02-project-intelligence/SKILL.md`

必须执行规则：`test_account_source,scope,role,tenant,redacted_secret`。

输出字段：`project_inventory.test_accounts[]`。

进入 manifest 字段：`project_inventory_v2.accounts`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/test-accounts.json`。
