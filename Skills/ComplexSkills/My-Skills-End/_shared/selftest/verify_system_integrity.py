...
STEP_FUNCTION_NAMES = [
    ...
    'check_candidate_and_dynamic_bridge', 'check_next_stage_fixes', 'check_extreme_reverse_judgement',
    'check_v43_completion', 'check_dashboard', 'check_tool_health', 'check_top_tier_completion_v2'
]


def check_top_tier_completion_v2() -> None:
    required = [
        ROOT / 'skills/06-dynamic-browser-burp-mcp/scripts/playwright_full_interaction_crawler.py',
        ROOT / 'skills/06-dynamic-browser-burp-mcp/scripts/role_tenant_authz_matrix_builder.py',
        ROOT / 'skills/07-vulnerability-hunting-engine/scripts/variant_expansion_engine.py',
        SHARED / 'quality/final_claim_guard.py',
        SHARED / 'evidence/EVIDENCE_MANIFEST_SCHEMA.v5.overlay.json',
        SHARED / 'top_tier/TOP_TIER_COMPLETION_CONTRACT.v2.md',
        SHARED / 'tests/smoke/top_tier_completion_gate_smoke_test.py',
    ]
    for path in required:
        check(path.exists(), f'top-tier completion v2 component missing: {path.relative_to(ROOT)}')
    cp = subprocess.run([sys.executable, str(SHARED / 'tests/smoke/top_tier_completion_gate_smoke_test.py')], text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=60, env=subprocess_env())
    try:
        result = json.loads(cp.stdout)
    except Exception as exc:
        ERRORS.append(f'top-tier completion smoke non-json: {exc}: {cp.stderr[:300]}')
        return
    check(cp.returncode == 0 and result.get('passed') is True and result.get('blocked_confirmed_claim') is True, f'top-tier completion smoke failed: {result}')


def _run_step_direct(step: str) -> int:
    global ERRORS, WARNINGS
    ERRORS = []
    WARNINGS = []
    if step not in STEP_FUNCTION_NAMES:
        print(json.dumps({'step': step, 'passed': False, 'errors': [f'unknown step: {step}'], 'warnings': []}, ensure_ascii=False, indent=2), flush=True)
        return 2
    try:
        globals()[step]()
    except Exception as exc:
        ERRORS.append(f'{step} raised {exc.__class__.__name__}: {exc}')
    result = {'step': step, 'passed': not ERRORS, 'errors': ERRORS, 'warnings': WARNINGS}
...
def main() -> int:
    if len(sys.argv) == 3 and sys.argv[1] == '--step':
        return _run_step_direct(sys.argv[2])
    all_errors: list[str] = []
    all_warnings: list[str] = []
    step_results: list[dict[str, Any]] = []
    for step in STEP_FUNCTION_NAMES:
        try:
            cp = subprocess.run([sys.executable, str(Path(__file__).resolve()), '--step', step], text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=150, env=subprocess_env())
        except subprocess.TimeoutExpired:
            err = f'{step} timed out after 150 seconds'
            all_errors.append(err)
            step_results.append({'step': step, 'passed': False, 'errors': [err], 'warnings': []})
            continue
        try:
            result = json.loads(cp.stdout)
        except Exception as exc:
            result = {'step': step, 'passed': False, 'errors': [f'{step} produced non-json selftest output: {exc}: {cp.stderr[:300]}'], 'warnings': []}
        if cp.returncode != 0 and result.get('passed') is True:
            result['passed'] = False
            result.setdefault('errors', []).append(f'{step} exited with code {cp.returncode}')
        step_results.append(result)
        all_errors.extend([f'{step}: {e}' for e in result.get('errors') or []])
        all_warnings.extend([f'{step}: {w}' for w in result.get('warnings') or []])
    # Final cleanup so packaging does not include transient bytecode.
    for d in list(ROOT.rglob('__pycache__')):
        shutil.rmtree(d, ignore_errors=True)
    for f in list(ROOT.rglob('*.pyc')):
        try: f.unlink()
    ...
    result = {
        'schema_version': 'system_integrity_selftest_subprocess_v2',
        'passed': not all_errors,
        'errors': all_errors,
        'warnings': all_warnings,
        'steps': step_results,
        'contract_count': len(list((ROOT / 'skills').glob('*/EXECUTION_CONTRACT.md'))),
        'canonical_template_count': len(load_json(SHARED / 'template_index.json').get('templates', [])),
        'canonical_replay_fixture_count': len(list((SHARED / 'tests' / 'fixtures').glob('C[0-9][0-9]-*.json'))),
    }
    print(json.dumps(result, ensure_ascii=False, indent=2), flush=True)
