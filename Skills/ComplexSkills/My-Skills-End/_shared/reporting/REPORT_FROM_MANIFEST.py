...
def generate(m,q):
    ...
    lines=[f"# {tid} Security Finding: {m.get('candidate_id')}", '', f"- Renderer: {renderer.relative_to(ROOT) if renderer.exists() else 'missing'}", f"- Final status: {m.get('final_status')}", f"- Quality gate: passed={q.get('passed')} total={q.get('total')}", f"- Authorized scope: {m.get('authorization_scope')}", '', '## Route and object context', f"- Route: `{m.get('method')} {m.get('route')}`", f"- Parameter: `{m.get('parameter')}`", f"- Auth context: `{esc(m.get('auth_context'))}`", f"- Tenant context: `{esc(m.get('tenant_context'))}`", '', '## Template-specific evidence']
