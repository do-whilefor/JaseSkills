...
## Anti-lazy and browser-interaction routing requirement

When a task involves frontend, JS audit, dynamic validation, severe vulnerability hunting or Skills capability scoring, the orchestrator must route to 05 and 06 for the lazy asset ledger and browser interaction matrix. A final report must not claim complete frontend coverage unless `_shared/quality/anti_lazy_browser_proof_gate.py` passes. `runtime_missing` is a valid blocked state, not a successful dynamic validation.
