...
- 任务涉及 JS、bundle、source map、CDN、前端路由、签名函数、隐藏接口、运行时请求时必须触发。
...
- `js_asset_inventory.json`
- `sourcemap_index.json`
- `runtime_endpoint_map.json`
...
- 脚本：`skills/05-js-audit-runtime/scripts/minimal_js_runtime_extractor.py`
- 输入：本机授权项目目录。
- 输出：`runs/<run_id>/js_asset_inventory.minimal.json` 或用户指定 `--out` 路径。
- 安全边界：只读扫描源码和构建产物；不会访问网络、不会执行项目代码、不会修改目标项目。
- 证据状态：该 extractor 输出为 candidate evidence，不得直接把候选点标记 confirmed。
- smoke test：由 `_shared/selftest/verify_system_integrity.py` 创建临时 fixture 并断言输出存在。


## Added contract: lazy asset proof

`lazy_js_asset_discovery.py` is mandatory for frontend audits. Missing dynamic imports, source maps, service workers, build manifests or API-client evidence must be recorded as an evidence gap, not silently ignored.
