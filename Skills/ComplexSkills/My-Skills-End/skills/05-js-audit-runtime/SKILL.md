...
## Lazy JS asset discovery hard requirement

Before saying JS collection is complete, execute or require `scripts/lazy_js_asset_discovery.py`. The output must cover dynamic import, framework lazy routes, build manifests, source maps, service workers, HTML preload/prefetch, frontend routes, API clients, GraphQL, WebSocket/SSE, feature flags and hidden route signals. The detailed checklist is `skills/05-js-audit-runtime/LAZY_JS_ASSET_DISCOVERY_MATRIX.md`. Static discovery alone is candidate evidence and must be paired with browser-triggered coverage from 06 before dynamic claims.
