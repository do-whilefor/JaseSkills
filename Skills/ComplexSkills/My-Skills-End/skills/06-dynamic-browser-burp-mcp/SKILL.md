...
## Browser interaction coverage hard requirement

Before saying dynamic browser validation is complete, execute or require `scripts/browser_interaction_coverage_matrix.py`. The output must cover click, scroll, input, hover, menu expansion, tab switching, modal opening, search, pagination, form validation, deep route refresh, error pages, role switching and tenant switching, or explicitly record why an action is not applicable. The detailed checklist is `skills/06-dynamic-browser-burp-mcp/BROWSER_INTERACTION_COVERAGE_MATRIX.md`. If Playwright/browser/Burp/MCP is unavailable, output `runtime_missing` and block dynamic-validation claims.

## Real browser full-interaction crawler addendum

Use `scripts/playwright_full_interaction_crawler.py` for authorized local/browser evidence. Default scope is file:// and localhost only. It records DOM, screenshot, network, HAR, console, storage keys, JS chunks and API calls. Click execution is opt-in with `--execute-clicks`; without execution, click coverage remains planned-only and cannot support a dynamic-complete claim.

Use `scripts/role_tenant_authz_matrix_builder.py` to generate role/tenant negative controls before promoting authz, IDOR, tenant, admin, token, GraphQL or WebSocket claims.
