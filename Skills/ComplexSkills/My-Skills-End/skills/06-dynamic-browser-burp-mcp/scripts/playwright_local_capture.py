...
def run(args):
    ...
    if not allowed(args.url): return {'schema_version':'dynamic_browser_role_matrix_result_v4.1','passed':False,'error':'url_not_allowed','policy':'localhost_or_file_only','url':args.url}
    availability=check(); captures=[]; browser_errors=[]
    for r in roles:
        try:
            if args.browser and availability['python_playwright_package']:
    ...
    result={'schema_version':'dynamic_browser_role_matrix_result_v4.1','url':args.url,'non_destructive':True,'policy':'localhost_or_file_only_browser_har_screenshot_role_tenant_matrix','browser_requested':bool(args.browser),'availability':availability,'capture_count':len(captures),'captures':captures,'negative_controls':neg,'browser_errors':browser_errors,'passed':not browser_errors or bool(captures)}
