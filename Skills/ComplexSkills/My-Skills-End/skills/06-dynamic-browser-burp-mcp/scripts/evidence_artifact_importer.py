...
def import_har(path: str) -> list[dict[str,Any]]:
    ...
    for e in (((obj or {}).get('log') or {}).get('entries') or []):
        ...
        entries.append({
            'source':'burp_or_har','method':req.get('method'),'url':req.get('url'),'status':res.get('status'),
            'mimeType':(res.get('content') or {}).get('mimeType'),
            'request_headers':{h.get('name'):h.get('value') for h in req.get('headers') or [] if h.get('name')},
            'response_headers':{h.get('name'):h.get('value') for h in res.get('headers') or [] if h.get('name')},
