...
## Regression requirement for lazy/browser gates

Regression must include `_shared/tests/smoke/anti_lazy_browser_gate_smoke_test.py`, the lazy JS asset ledger schema and the browser interaction coverage schema. Dashboard output should expose lazy ledger status, browser matrix status, `runtime_missing` blockers and role/tenant coverage gaps.

## Top-tier completion smoke test

Run `_shared/tests/smoke/top_tier_completion_gate_smoke_test.py` after packaging. It proves that unsupported confirmed claims are blocked and that role/tenant and variant-expansion obligations are generated.
