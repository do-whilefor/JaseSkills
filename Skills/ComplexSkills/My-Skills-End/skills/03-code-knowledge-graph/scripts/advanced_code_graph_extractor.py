...
def scan_text(rel:str,text:str,suffix:str,lang:str)->dict[str,list]:
    routes=[]; api=[]; sinks=[]; authz=[]; tenant=[]; middleware=[]; calls=[]; symbols=[]; imports=scan_imports(rel,text,lang)
    ...
    return {'routes':routes,'api_clients':api,'sinks':sinks,'authz_boundaries':authz,'tenant_boundaries':tenant,'middleware_boundaries':middleware,'calls':calls,'symbols':symbols,'imports':imports}

def merge_ts(out,ts):
    if not ts.get('ready'): return
    for k in ['routes','imports','exports','dynamic_imports','calls','api_clients','authz_boundaries','tenant_boundaries','sinks','edges']:
...
def extract(root:Path)->dict[str,Any]:
    ...
    for k in ['imports','exports','symbols','calls','routes','api_clients','sinks','authz_boundaries','tenant_boundaries','middleware_boundaries']:
    ...
    out['edge_count']=len(out['edges']); out['node_count']=len(out['nodes']); out['route_count']=len(out['routes']); out['sink_count']=len(out['sinks'])
    return out
