...
- 任务涉及 AST、路由到 handler、调用图、source/sink、代码知识图谱、安全知识图谱时必须触发。
...
- 脚本：`skills/03-code-knowledge-graph/scripts/minimal_code_graph_extractor.py`
- 输入：本机授权项目目录。
- 输出：`runs/<run_id>/security_graph.minimal.json` 或用户指定 `--out` 路径。
