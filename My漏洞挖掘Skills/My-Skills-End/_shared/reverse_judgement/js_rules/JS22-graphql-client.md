# JS22-graphql-client GraphQL client 识别

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`operation/fragment/persisted query parser`。

输出字段：`graphql_clients[]`。

候选漏洞联动：`C17`。

进入 manifest 字段：`js_graph.graphql_clients`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-graphql-client.json`。
