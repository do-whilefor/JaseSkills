# JS28-button-permission 按钮权限识别

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`permission key/button visibility scanner`。

输出字段：`button_permissions[]`。

候选漏洞联动：`C02,C05`。

进入 manifest 字段：`js_graph.frontend_permissions`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-button-permission.json`。
