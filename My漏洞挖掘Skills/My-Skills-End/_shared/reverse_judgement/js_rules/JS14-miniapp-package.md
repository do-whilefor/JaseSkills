# JS14-miniapp-package 小程序包

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`miniapp config/pages/API scanner`。

输出字段：`miniapp_surfaces[]`。

候选漏洞联动：`C03,C22`。

进入 manifest 字段：`js_graph.miniapp`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-miniapp.json`。
