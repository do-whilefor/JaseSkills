# JS11-framework-bundles Next/Nuxt/Vite/Webpack/Rollup/Angular/Vue/React/Svelte 产物

负责 skill：`skills/05-js-audit-runtime/SKILL.md`

AST / parser 要求：`framework artifact classifiers`。

输出字段：`framework_artifacts[]`。

候选漏洞联动：`C03,C22`。

进入 manifest 字段：`js_graph.framework_artifacts`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/js-framework-bundles.json`。
