# INFO19-orm-model ORM model 识别

负责 skill：`skills/02-project-intelligence/SKILL.md`

必须执行规则：`model_file,model_name,owner_field,tenant_field,raw_query_flag`。

输出字段：`project_inventory.orm_models[]`。

进入 manifest 字段：`project_inventory_v2.orm`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/orm-owner-tenant.json`。
