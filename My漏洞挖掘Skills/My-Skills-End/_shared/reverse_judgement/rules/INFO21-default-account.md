# INFO21-default-account 默认账号识别

负责 skill：`skills/02-project-intelligence/SKILL.md`

必须执行规则：`account_source,credential_redacted,role,environment_only`。

输出字段：`project_inventory.accounts[]`。

进入 manifest 字段：`project_inventory_v2.accounts`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/default-accounts-redacted.json`。
