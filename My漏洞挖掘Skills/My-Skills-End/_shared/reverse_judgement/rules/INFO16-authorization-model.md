# INFO16-authorization-model 授权模型识别

负责 skill：`skills/04-attack-surface-mapping/SKILL.md`

必须执行规则：`rbac,abac,acl,owner_check,policy_path`。

输出字段：`attack_surface.authorization_model[]`。

进入 manifest 字段：`surface_manifest_v2.authorization`。

测试样例：`_shared/tests/extreme_reverse_audit/fixtures/authorization-rbac-abac.json`。
