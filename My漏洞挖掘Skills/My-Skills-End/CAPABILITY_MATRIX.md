...
| 权限模型分析 | skills/03; skills/07 | candidate engine + detectors | fixtures/replay | candidate replay passed | PARTIAL | 多角色矩阵仍依赖动态账号 |
| 多角色访问差异分析 | skills/06; _shared/tests/high_risk_replay | dynamic scripts | high_risk_replay_runner.py | 28 cases passed | PARTIAL | 真实浏览器 runtime 当前 blocked |
...
| 动态验证 | skills/06 | playwright/capture scripts | playwright_runtime_manager.py | Playwright browser missing | PARTIAL | 浏览器未安装，不能声称 ready |
| 证据保存 | _shared/evidence schemas; skills/06 scripts | evidence_capture_bridge.py; tools/evidence_builder.py | fixtures/selftest | selftest passes | IMPLEMENTED | 动态 request/response 需真实采集 |
