...
| **Coordinator** | Orchestrator | One per target | Scope, OUTPUT_DIR, target list/IDs, mounted skills (lazy) | env vars not set (orchestrator handles) | `attack-chain.md`, `experiments.md`, `findings/`, `artifacts/`, `tools/`, `reports/`, `AUDIT_SUMMARY` | `AskUserQuestion`, `/skill-update`, `/slack-send`, anything outside OUTPUT_DIR | AUDIT_SUMMARY emitted; coordinator exits |
