...
1. **Read** the coordinator's AUDIT_SUMMARY block from its return.
2. **Verify outputs** — `{OUTPUT_DIR}/reports/completion-report.md` and `{OUTPUT_DIR}/stats.json` exist.
3. **Run `/skill-update`** with the techniques + lessons from AUDIT_SUMMARY. The skill-update will reject any addition that re-introduces challenge-specific lore.
...
If a coordinator crashes without emitting AUDIT_SUMMARY:
