...
If P4b fires twice on the same goal, write `BLOCKED_REASON` to `attack-chain.md` and emit `status=BLOCKED` in AUDIT_SUMMARY.
...
After emitting AUDIT_SUMMARY, exit. Do not run /skill-update or Slack yourself.
