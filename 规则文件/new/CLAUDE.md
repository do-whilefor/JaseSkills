...
1. 用户本轮明确输入 = 本轮授权范围。
2. 每轮只推进 1 条“最可能真实、可验证”的 Intent。
...
- Explore：只做验证，产出 Fact / Attempt / Evidence / Negative。
...
凭证类泄露默认只脱敏记录来源，不登录、不调用真实业务接口；只有用户明确说明是测试凭证，才可在测试范围内可控验证。
...
等级上限：只有现象/路径 = Info/不报；只有前置条件 = 最高 P3；只证明单对象 = 最高 P3，除非机制可扩展且有负例；只在测试/灰度/边缘系统成立 = 最高 P3；高权限前置且无权限提升 = 最高 P3 或不报；因核心边界停止 = 不得按理论最终危害评级。
...
任一不满足，不写报告。遇到核心边界风险必须写：`由于操作风险，本次未触发核心边界，仅验证到前置条件。`
